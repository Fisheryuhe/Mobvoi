...
import com.mobvoi.be.common.util.TimeUtil;
...
public class HomeLocationHandler implements IHandler {
  
  private static final MobvoiLog LOG = new MobvoiLog(HomeLocationHandler.class);
  ...
  private static final double percentageThreshold = 0.7;
  ...
  protected boolean isValidHour(int weekday, int hour) {
  ...
  public KeyValueData handle(String key, List<Object> datas) throws Exception {
    ...
    for (Cluster cluster : clusters) {
      double weight = 0;
      for (DataPoint point : cluster.points) weight += point.weight;
      AddressDataPoint point = (AddressDataPoint)geoKMeans.getMinDistPoint(cluster);
      LOG.debug(point.getFormatedAddress() + " : time " + weight);
    }
    ...
    LocationDetailData value = new LocationDetailData(point.getFormatedAddress(), point.getCity(),
        pred_lat, pred_lng);
  ...
  private double[] getWeight(List<LocationData> datas) {
    ...
    for (int i = 0; i < datas.size(); i++) {
      if (i > 0) {
        double tmp_weight = (datas.get(i).timestamp - datas.get(i - 1).timestamp) / TimeUtil.MINUTE_MS / 2;
        tmp_weight = Math.min(maxWeight, tmp_weight);
        tmp_weight = Math.max(minWeight, tmp_weight);
        weights[i] += tmp_weight;
      }
      if (i < datas.size() - 1) {
        double tmp_weight = (datas.get(i + 1).timestamp - datas.get(i).timestamp) / TimeUtil.MINUTE_MS / 2;
        tmp_weight = Math.min(maxWeight, tmp_weight);
        tmp_weight = Math.max(minWeight, tmp_weight);
        weights[i] += tmp_weight;
      }
